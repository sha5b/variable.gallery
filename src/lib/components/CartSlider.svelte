<script>
    ...
    // Use the store for cart slider open state
    let isOpen;
    ...
    function closeCart() {
      toggleCartSlider(); // Toggle the cart slider state
    }
    ...
    function handleCheckout() {
      ...
      goto('/checkout'); // Navigate to the checkout page
    }
  </script>
          ...
          <ul class="space-y-8">
            {#each $cart as item (item.id)}
              <li class="flex items-start">
                <!-- Always display the product's main thumbnail (use item.thumbnail from the product level) -->
  ...
  <style>
    .cart-content {
      max-height: calc(100vh - 160px); /* Adjust height to account for padding and button */
    }
  </style>
